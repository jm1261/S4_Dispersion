    ...
    for thickness in thicknesses:
        for radius in radii:
            ...
            for index, period in enumerate(periods):
                ...
                x_range = np.arange(start=sim_params['Wav Range'][0],
                                    stop=sim_params['Wav Range'][1],
                                    step=sim_params['Wav Range'][2])
                for x in x_range:
...
for index, selected_file in enumerate(data_files):
    file = os.path.join(dispersion_dir, selected_file)
    _, _, _, file_name = io.dispersion_in(file)
    print(f'\nNow processing: {file_name}')
